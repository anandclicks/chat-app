  ...
  return (
    <>
      {msgData?.images?.length >= 1 && (
        <div className='sendMsgWrapper h-[250px] w-full flex justify-start items-center mb-2'>
           <div className="relative max-w-[250px] h-[250px] rounded-xl overflow-hidden">
            ...
            {msgData?.images.length >= 2 && (
              <div className='bg-[#00000050] absolute z-10 top-0 h-full w-full flex justify-center items-center text-white text-xl cursor-pointer'>{msgData?.images.length - 1}+</div>
            )}
            </div>
        </div>
      )}
